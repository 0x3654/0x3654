# [🇺🇸 English](#english) | [🇷🇺 Русский](#русский)

---

<a id="english"></a>
# Hi, I'm Mihail (0x3654)

📍 **Moscow → Tbilisi** | **Business Analyst** | Tech tinkerer | Cat enthusiast
Dropped an AirPod in coffee. once ☕🎧 | Turning absurdity into tweets, one day at a time

![1C](https://img.shields.io/badge/1C-Enterprise-orange?style=flat-square&logo=1c)
![Ansible](https://img.shields.io/badge/Ansible-automation-red?style=flat-square&logo=ansible)
![CI/CD](https://img.shields.io/badge/CI%2FCD-pipelines-green?style=flat-square)
![macOS](https://img.shields.io/badge/macOS-power%20user-999999?style=flat-square&logo=apple)
![Python](https://img.shields.io/badge/Scripting-Python-blue?style=flat-square&logo=python)
![Bash](https://img.shields.io/badge/Shell-Bash-4EAA25?style=flat-square&logo=gnu-bash)

---

## About Me

- 💼 Profession: Business Analyst, building bridges between chaos and clarity
- 🧠 Exploring the intersection of business, data, tech, and automation
- 🧑‍💻 1C, Ansible, CI/CD — from chaos to order
- 🍎 macOS power user with a Linux soul
- 🥗 Optimizing brainpower and macros — doctor-approved, muscle-safe
- 🐈 Cat lover, amused by their endless antics
- 🤖 Tech tinkerer & occasional code dabbler
- 🐦 Turning absurdity into tweets, one day at a time [@0x3654 on X](https://x.com/0x3654)

---

## Currently Exploring

- 🔧 DevOps & CI/CD pipelines — automating everything that moves
- 🤖 AI agents and MCP servers
- 📦 Infrastructure as code

---

## Current Projects

- 📱 **[untilwall](https://github.com/0x3654/untilwall)** - Life calendar wallpaper generator
- 🤖 **[telegram-mcp&rest api](https://github.com/0x3654/telegram)** - Telegram mtproto to Rest api or MCP server
- 🔍 **[GISP](https://github.com/0x3654/gisp)** - Search through the register of Russian industrial products of the Ministry of Industry and Trade in a chat format

---

## Fun Facts

- ☕🎧 Dropped an AirPod in coffee. Once.
- 🍎🐧 macOS power user with a Linux soul — the best of both worlds
- 🐈 Cats are my pair programmers — they mostly sleep through the code reviews
- 💪 Optimizing brainpower and macros — doctor-approved, muscle-safe

---

## Philosophy

> "Impossible is temporary" — I break complex problems into small, solvable pieces. Currently exploring how DevOps and AI change everything about business automation.

---

## GitHub Stats

![GitHub Stats](https://gh-card.dev/repos/0x3654/0x3654/full-width)

---

## Connect with Me

- [X (Twitter)](https://x.com/0x3654)

---

<a id="русский"></a>
# 👋 Привет, я Михаил (0x3654)

📍 **Москва → Тбилиси** | **Бизнес-аналитик** | Любитель техники | Любитель кошек |
Уронил AirPod в кофе. Однажды ☕🎧 | Превращаю абсурд в твиты, день за днём

![1C](https://img.shields.io/badge/1C-Enterprise-orange?style=flat-square&logo=1c)
![Ansible](https://img.shields.io/badge/Ansible-automation-red?style=flat-square&logo=ansible)
![CI/CD](https://img.shields.io/badge/CI%2FCD-pipelines-green?style=flat-square)
![macOS](https://img.shields.io/badge/macOS-power%20user-999999?style=flat-square&logo=apple)
![Python](https://img.shields.io/badge/Scripting-Python-blue?style=flat-square&logo=python)
![Bash](https://img.shields.io/badge/Shell-Bash-4EAA25?style=flat-square&logo=gnu-bash)

---

## О себе

- 💼 Профессия: бизнес-аналитик, строю мосты между хаосом и порядком
- 🧠 Исследую пересечение бизнеса, данных, технологий и автоматизации
- 🧑‍💻 1С, Ansible, CI/CD — навожу порядок в хаосе
- 🍎 Пользователь macOS с душой Linux
- 🥗 Балансирую мозговую работу и питание — по одобрению врача, безопасно для мышц
- 🐈 Любитель кошек и их бесконечных выходок
- 🤖 Техноковыряка и иногда балуюсь кодом
- 🐦 Превращаю абсурд в твиты, день за днём [@0x3654 на X](https://x.com/0x3654)

---

## Сейчас изучаю

- 🔧 DevOps и CI/CD пайплайны — автоматизирую всё, что движется
- 🤖 AI-агенты и MCP серверы
- 📦 Infrastructure as Code

---

## Текущие проекты

- 📱 **[intilwall](https://github.com/0x3654/untilwall)** — Генератор обоев «Жизненный календарь»
- 🤖 **[telegram-mcp&rest api](https://github.com/0x3654/telegram)** — Telegram mtproto в Rest API или MCP сервер
- 🔍 **[GISP](https://github.com/0x3654/gisp)** — Поиск по реестру промышленных продуктов Минпромторга в формате чата

---

## Забавные факты

- ☕🎧 Уронил AirPod в кофе. Однажды.
- 🍎🐧 Пользователь macOS с душой Linux — лучшее из двух миров
- 🐈 Кошки — мои pair-программисты, в основном спят во время code review
- 💪 Балансирую мозговую работу и питание — по одобрению врача, безопасно для мышц

---

## Философия

> "Невозможно — это не навсегда" — ломаю сложные задачи на мелкие решаемые части. Сейчас исследую, как DevOps и ИИ меняют всё в бизнес-автоматизации.

---

## Статистика GitHub

![GitHub Stats](https://gh-card.dev/repos/0x3654/0x3654/full-width)

---

## Контакты

- [X (Twitter)](https://x.com/0x3654)

---

[↑ Language selector](#-english---русский)
